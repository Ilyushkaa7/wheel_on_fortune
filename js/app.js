...
function buildItemsNormal() {
    ...
    if (mode === 'range') {
        ...
        const rawExclude = excludeInput.value
            .split(',')
...
function loadGroupIT(index) {
    ...
    const rawExclude = excludeInputIT.value
        .split(',')
        .map(s => parseInt(s.trim()))
        .filter(n => !isNaN(n));
...
state.settings.spinsPerSeries =
    parseInt(spinsRange.value);

biasEnabled.addEventListener('change', () => {
    state.settings.biasEnabled =
        biasEnabled.checked;

    biasTargetID.disabled =
        !biasEnabled.checked;
...
if (btnLegal && legalModal && closeLegalInfo) {

    btnLegal.addEventListener('click', () => {
        legalModal.classList.remove('hidden');
    });

    closeLegalInfo.addEventListener('click', () => {
...
async function init() {
    ...
    try {
        ...
        function openDropdown() {
            ...
            const rect =
                groupDropdownBtn.getBoundingClientRect();

            groupDropdownMenu.style.top =
                rect.bottom + 6 + 'px';

            groupDropdownMenu.style.left =
                rect.left + 'px';

            groupDropdownMenu.style.width =
                rect.width + 'px';
        ...
        console.warn('Ошибка загрузки groups.json', e);
    }
